...
def test_pkg_files_no_CLI(
    testing_path: Path, files_no_CLI: Dict[str, List[str]]
):
    root_dir = choose_object_for_sys(
        testing_path / "pkgNoCLI", testing_path / "pkgNoCLIWin"
    )
    src_dir = choose_object_for_sys(
        root_dir / "pkgNoCLI", root_dir / "pkgNoCLIWin"
    )
...
def test_pkg_files_with_CLI(
    testing_path: Path, files_with_CLI: Dict[str, List[str]]
):
    root_dir = choose_object_for_sys(
        testing_path / "pkgWithCLI", testing_path / "pkgWithCLIWin"
    )
    src_dir = choose_object_for_sys(
        root_dir / "pkgWithCLI", root_dir / "pkgWithCLIWin"
    )
    tests_dir = root_dir / "tests"
